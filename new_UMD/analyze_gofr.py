...
def headerfile(firstfile):
    ...
    with open(firstfile, 'r') as f:
        header = f.readline()
    header = header.strip('dist') #Will remove d, i, s and t from sides
    header = re.sub('(Int\([A-Za-z1-9-]*\))', ' ', header).split()#Will remove the first argument and replace it with the second
