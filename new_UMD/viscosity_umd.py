...
def BuildStressTensor(AllSnapshots,firststep):#Actually it returns the list of the values over each time step of the 3 components of the stress tensor separately + the mean of these
...
#        AllSnapshots = [cr.Lattice]
        ...
        temperature = 0.0
        #print("Voici : ",Viscosity_calculation(AllSnapshots, firststep))
...
#            print('current temperature at step ',ii,' is:', AllSnapshots[ii].temperature)
        ...
        if (length+firststep+1>=len(AllSnapshots)):
            if(not(mod)):
                print('The length requested (', length,')  is too long for the whole duration of the trajectory(', len(AllSnapshots),') if begun from step',firststep)
            length=len(AllSnapshots)-firststep-2
            mod=True
        
        if(mod):
            print('Will impose the length of the trajectory :',length)
