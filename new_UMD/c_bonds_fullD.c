...
bool specificity(double x, double y, double z, const double *specifics){
...
int* compute_fBonds_specific(const double *MySnapshot, const double *BondTable, const int *CrystalTypes, const int nAtoms, const int ntypat, const double acell0, const double acell1, const double acell2, const int nCells, const double *specifics){
    ...
    int* Bonding;
    int* nBonding;
    int* Ats;//table containing the atoms in a central cell
    int* Coord;//table containing the coordinating atoms in the 27 surrounding cells
...
//    MySnapshot = malloc((3*nAtoms)*sizeof(double));
  ...
  //  CrystalTypes = malloc(nAtoms * sizeof(int));
    ...
    Ats = calloc(M,sizeof(int));
    Coord = calloc(M*27,sizeof(int));
...
if(Mesh==NULL){fprintf(stderr,"Mem alloc failure");exit(1);}
    ...
    for(int iat=0 ; iat<nAtoms ; iat++) {
	    ...
	    for(int iatom=0 ; iatom<index ; iatom++){
		currentAtom = Mesh[M*cellnum+iatom];
               	if(specificity(MySnapshot[3*currentAtom],MySnapshot[3*currentAtom+1],MySnapshot[3*currentAtom+2],specifics)){
			
		        Ats[iAt]=currentAtom;
			iAt++;
		}
		DicoAtoms[3*currentAtom]=-1;
            ...
            for(int i=-1 ; i<2 ; i++){
			    ...
			    for(int jatom = 0 ; jatom<indexCoord ; jatom++){
                 ...
                 for(int iatom = 0 ; iatom<iAt ; iatom++){
