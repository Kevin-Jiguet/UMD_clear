...
def clusteringnorings(BooleanMap,centralatoms,outeratoms): #compute the list of clusters centered on the atoms from the centeratoms list.
    #print ('ligands:',ligands)
...
#first build the bonding maps
        BondMap=[[0.0 for _ in range(MyCrystal.natom)] for _ in range(MyCrystal.natom)]
        BooleanMap=[[0 for _ in range(MyCrystal.natom)] for _ in range(MyCrystal.natom)]
        (BondMap,BooleanMap) = ComputeBondMapInput(MyCrystal,AllSnapshots[istep],ligands,BondTable) #Booleanmap says which atom can be considered as bonded with each one
#        print('BondMap : ', BondMap)
        if rings == 1:
            clusters.append(clustering(BooleanMap,ligands))
        elif rings == 0:
            clusters.append(clusteringnorings(BooleanMap,centralatoms,outeratoms)) #Contain the list of all the clusters for each snapshot
